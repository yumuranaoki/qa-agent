...
from .browser import browser_manager, get_browser_context
...
async def execute(
    ...
    async with browser_manager(initiate_session=initiate_session) as browser:
        browser_context = get_browser_context(
            browser=browser, allowed_domains=allowed_domains
        )
